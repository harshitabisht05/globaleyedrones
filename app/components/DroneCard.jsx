...
export default function DroneCard({ name, features, image }) {
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md hover:shadow-2xl transition transform hover:-translate-y-1 hover:border-blue-500 border border-transparent">
      ...
      <h3 className="text-2xl font-semibold mb-2" style={{ fontFamily: 'var(--font-heading)' }}>{name}</h3>
      <ul className="text-left text-sm list-disc list-inside text-gray-700 dark:text-gray-300" style={{ fontFamily: 'var(--font-body)' }}>
        {features.map((feat, i) => (
          <li key={i}>{feat}</li>
        ))}
      </ul>
